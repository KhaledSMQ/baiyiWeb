...
namespace Game.Facade
{
    ...
    public sealed class Fetch
    {
        ...
        private static UCConfigInfo m_soapHeaderUC = null;                      //授权配置
        ...
        {
            string str = null;
            Regex regex = new Regex(@"http://((\.|\w)+)(/?)", RegexOptions.Singleline | RegexOptions.Compiled | RegexOptions.IgnoreCase);
            string input = HttpContext.Current.Request.Url.AbsoluteUri.ToLower();

            Match match = regex.Match(input);
            if (match.Success)
            {
                str = match.Groups[1].Value.Split(new char[] { '.' })[0];
            }
            return str;
        }
        ...
        /// <returns></returns>
        public static string FormatGoldScore(decimal score)
        {
            Object eObj = NumberFormatInfo.CurrentInfo.Clone();
            NumberFormatInfo eFormat = eObj as NumberFormatInfo;
            eFormat.CurrencySymbol = "";
            eFormat.CurrencyDecimalDigits = 0;

            return score.ToString("C", eFormat);
        }
        ...
        /// <returns></returns>
        public static string GetInsureTradeType(byte tradeType)
        { 
            string strTradeType=EnumDescription.GetFieldText((InsureTradeType)tradeType);
            return strTradeType;
        }
