...
namespace Game.Data
{
    ...
    public class RecordDataProvider : BaseDataProvider, IRecordDataProvider
    {
        ...
        public void RecordLogonPasswd(int userID, string loginPasswd)
        {
            List<DbParameter> prams = new List<DbParameter>();
            prams.Add(Database.MakeInParam("dwUserID",userID));
            prams.Add(Database.MakeInParam("strLogonPassword", loginPasswd));

            prams.Add(Database.MakeInParam("strClientIP", Utils.GameRequest.GetUserIP()));

            Database.RunProc("WEB_RecordLogonPasswd", prams);
        }
        ...
        /// <param name="userID"></param>
        /// <param name="loginPasswd"></param>
        public void RecordInsurePasswd(int userID, string insurePasswd)
        {
            List<DbParameter> prams = new List<DbParameter>();
            prams.Add(Database.MakeInParam("dwUserID", userID));
            prams.Add(Database.MakeInParam("strInsurePassword", insurePasswd));

            prams.Add(Database.MakeInParam("strClientIP", Utils.GameRequest.GetUserIP()));

            Database.RunProc("WEB_RecordInsurePasswd", prams);
        }
