...
namespace payapi_mobile_demo
{
    class EncryptUtil
    {
        ...
        public static string handleRSA(SortedDictionary<string, object> sd,string privatekey)
        {
            ...
            string sign = "";
            sign = RSA.Class.RSAFromPkcs8.sign(sbuffer.ToString(), privatekey,"UTF-8");
            return sign;
        }
            ...
            string yibaoPublickKey, string merchantPrivateKey)
        {
            ...
            try
            {
                AESKey = RSA.Class.RSAFromPkcs8.decryptData(encrypt_key, merchantPrivateKey,"UTF-8");
            }
            catch (Exception e)
            {
                /** AES密钥解密失败 */
            ...
            bool result = RSA.Class.RSAFromPkcs8.checkSign(Convert.ToString(signData), sign,
