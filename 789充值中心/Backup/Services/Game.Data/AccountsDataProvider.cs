...
namespace Game.Data
{
    public class AccountsDataProvider : BaseDataProvider, IAccountsDataProvider
    {
        ...
        public string GetAccountsBySLDomain(string slDomain)
        {
            ...
            sqlQuery.Append("SELECT Accounts FROM AccountsInfo ")
                    .AppendFormat(" WHERE SLDomain = '{0}'", slDomain);

            return Database.ExecuteObject<AccountsInfo>(sqlQuery.ToString()) == null ? "" : Database.ExecuteObject<AccountsInfo>(sqlQuery.ToString()).Accounts.ToString();
        }
